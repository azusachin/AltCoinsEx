...
`./`               | `AltCoinsExCoin.conf` | User-defined [configuration settings](bitcoin-conf.md) for `acecnd` or `litecoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `acecnd.pid`          | Stores the process ID (PID) of `acecnd` or `litecoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `acecnd` or `litecoin-qt`; can be specified by `-debuglogfile` option
