...
---------|--------------------
Linux    | `$HOME/.AltCoinsExCoin/`
macOS    | `$HOME/Library/Application Support/AltCoinsExCoin/`
Windows  | `%APPDATA%\AltCoinsExCoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `AltCoinsExCoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `AltCoinsExCoin.conf` | User-defined [configuration settings](bitcoin-conf.md) for `acecnd` or `acec-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `acecnd.pid`          | Stores the process ID (PID) of `acecnd` or `acec-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `acecnd` or `acec-qt`; can be specified by `-debuglogfile` option
...
`acec-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
