...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an AltCoinsExCoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no altcoinsex.com: URI
    if(!uri.isValid() || uri.scheme() != QString("altcoinsex.com"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("altcoinsex.com:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
